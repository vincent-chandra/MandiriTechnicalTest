...
class MovieDetailsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        ListMoviesInteractor.fetchAPIMovieDetail(id: id) { data in
        ...
        ListMoviesInteractor.fetchAPIMovieVideo(id: id) { data in
            DispatchQueue.main.async {
                self.loadYoutube(videoID: data.results?[0].key ?? "-aAHfOQ7Rbo")
            }
        }
        
        ListMoviesInteractor.fetchAPIUserReview(page: currentPageReview, id: id) { data in
            DispatchQueue.main.async {
                self.arrReview = data
...
extension MovieDetailsViewController: UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        if (indexPath.row == arrReview.results!.count - 1){
            currentPageReview += 1
            ListMoviesInteractor.fetchAPIUserReview(page: currentPageReview, id: id) { data in
                DispatchQueue.main.async {
